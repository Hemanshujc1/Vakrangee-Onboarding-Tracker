        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 border border-gray-100 rounded-lg bg-gray-50 gap-3">
            ...
            <div className="flex items-center gap-3">
                {showToggle && (
                    <button
                    onClick={onToggle}
                    className={`p-1 rounded-md transition-colors ${isDisabled ? 'text-red-500 bg-black-500 hover:bg-red-100' : 'text-green-500 bg-green-50 hover:bg-green-100'}`}
                ...
                <div className="flex flex-col items-end">
                    ...
                    {status === 'VERIFIED' && verifiedByName && (
                        <span className="text-[10px] text-gray-500 mt-1">By: {verifiedByName}</span>
                    )}
                </div>
